#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════
# Vibe-Research 一键启停脚本（快速使用教程）
#
# ── 1. 首次使用：安装依赖 ─────────────────────────────────────
#   后端依赖：脚本首次 start 会自动安装 fastapi / uvicorn / requests，
#     以及可选数据源 akshare / mootdx / tushare；也可手动执行：
#       python3 -m pip install --break-system-packages fastapi uvicorn requests
#   前端依赖：首次 start 会自动执行 npm install
#     （等价命令：cd frontend && npm install）
#
# ── 2. 配置（可选）───────────────────────────────────────────
#   在项目根目录 .env 中写后端所需环境变量（每行 KEY=VALUE），
#   脚本会自动加载；已存在的同名环境变量不会被覆盖。
#
# ── 3. 启动 ──────────────────────────────────────────────────
#     ./run.sh start
#   启动后浏览器访问：  http://localhost:8901   （前端界面）
#   前端通过 http://127.0.0.1:8900 调用后端，日志写入 logs/
#
# ── 4. 常用命令 ──────────────────────────────────────────────
#     ./run.sh start     启动：后端 uvicorn (:8900) + 前端 Vite (:8901)
#     ./run.sh status    查看后端 / 前端 / 外网隧道运行状态
#     ./run.sh restart   停止后重启（不含外网隧道）
#     ./run.sh stop      停止全部服务（含外网隧道）
#     ./run.sh tunnel    启动 cloudflared 外网隧道（需已 start；本机装有 cloudflared）
#
# ── 5. 查看日志 / 调试 ───────────────────────────────────────
#     tail -f logs/backend.log    后端日志
#     tail -f logs/frontend.log   前端日志
#   后端启动失败时，查看： tail -20 logs/backend.log
#
# ── 6. 停止 ──────────────────────────────────────────────────
#     ./run.sh stop
# ═══════════════════════════════════════════════════════════════
