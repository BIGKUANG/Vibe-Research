...
_HANDLERS = {
    ...
    "query_reports": lambda a: _pick(astock.eastmoney_reports(str(a["code"]), max_pages=1)["items"],
                                     ("title", "publishDate", "orgSName", "emRatingName"), 15),
    "query_news": lambda a: _pick(astock.stock_news(str(a["code"]), limit=15)["items"],
                                  ("新闻标题", "发布时间", "文章来源"), 15),
